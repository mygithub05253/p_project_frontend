 ...
 * 
 * 유스케이스: UC-3 비밀번호 재설정
 * 시퀀스: LoginPage -> ForgotPasswordPage -> 이메일 입력 -> 인증 코드 발송 -> 인증 코드 입력 -> 새 비밀번호 입력 -> resetPassword API 호출 -> 로그인 페이지로 이동
 * 
 * 주요 기능:
 * - 이메일 입력 및 인증 코드 발송
 * - 인증 코드 입력 및 검증
...
export function ForgotPasswordPage({ onBackToLogin }: ForgotPasswordPageProps) {
  ...
  /**
   * 유스케이스: UC-3-1 인증 코드 발송
   * 시퀀스: 이메일 입력 -> sendVerificationCode API 호출 -> 인증 코드 발송 -> 인증 코드 입력 단계로 이동
   * 
   * 사용자가 입력한 이메일로 비밀번호 재설정을 위한 인증 코드를 발송하고
   * 인증 코드 입력 단계로 전환
   */
  const handleSendCode = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setCodeSent(true);
      setStep('verify'); // 인증 코드 입력 단계로 전환
    } catch (err) {
  ...
  /**
   * 유스케이스: UC-3-2 비밀번호 재설정
   * 시퀀스: 인증 코드 및 새 비밀번호 입력 -> resetPassword API 호출 -> 비밀번호 재설정 완료 -> 로그인 페이지로 이동
   * 
