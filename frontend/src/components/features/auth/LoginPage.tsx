 ...
 * 
 * 유스케이스: UC-16 로그인
 * 시퀀스: 사용자 입력 -> LoginPage -> 입력 검증 -> login API 호출 -> 토큰 저장 -> App -> 일기장 화면 전환
 * 
 * 주요 기능:
 * - 이메일과 비밀번호 입력 폼 제공
 * - 입력값 유효성 검증
...
export function LoginPage({ onLoginSuccess, onBack, onSignup, onForgotPassword }: LoginPageProps) {
  ...
  /**
   * 유스케이스: UC-16 로그인 처리
   * 시퀀스: 폼 제출 -> 입력 검증 -> login API 호출 -> 토큰 저장 -> onLoginSuccess 콜백 호출
   * 
   * 사용자가 입력한 이메일과 비밀번호로 로그인을 시도하고
   * 성공 시 토큰이 저장되며 일기장 화면으로 전환
   */
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // login API 호출 (이메일을 username으로 사용)
      const response = await login({ username: email, password });
      
      // 성공 시 토큰과 사용자 정보는 login 함수 내부에서 저장됨
      onLoginSuccess();
    } catch (err) {
