 ...
 * 
 * 유스케이스: UC-28 일별 통계 조회, UC-29 차트 통계 조회
 * 시퀀스: DiaryBook -> EmotionStatsPage -> fetchDailyStats API 호출 -> 일별 통계 데이터 로드 -> 캘린더/타임라인/차트 뷰로 표시
 * 
 * 주요 기능:
 * - 월별 일기 통계 조회 및 표시 (캘린더 뷰, 타임라인 뷰)
 * - 기간별 감정 변화 차트 표시 (주간/월간, 선 그래프/막대 그래프)
 * - 날짜 클릭 시 해당 날짜의 일기 상세 조회
 * - 월별 네비게이션
 */
import { useState, useEffect } from 'react';
import { ChevronLeft, ChevronRight, Calendar, BarChart3, Loader2, TrendingUp } from 'lucide-react';
import { fetchDailyStats } from '../../../services/diaryApi';
import type { DailyStats } from '../../../services/diaryApi';
...
export function EmotionStatsPage({ onDateClick, refreshKey }: EmotionStatsPageProps) {
   ...
   * 시퀀스: 월 변경/viewMode 변경/refreshKey 변경 -> EmotionStatsPage -> fetchDailyStats API 호출 -> 일별 통계 데이터 로드 -> 캘린더/타임라인에 표시
   * 
   * 표시 중인 월이 변경되거나 뷰 모드가 변경될 때
   * 해당 월의 일별 통계 데이터를 조회하여 캘린더 또는 타임라인 뷰에 표시
   */
  useEffect(() => {
    // 차트 모드가 아닐 때만 일별 통계 데이터 로드
    if (viewMode !== 'chart') {
      loadMonthData();
    }
      ...
      <div className="flex-1 relative">
        ...
        <div className="relative p-8 h-full overflow-y-auto">
          <div className="space-y-6">
            ...
            {/* Chart View */}
            {viewMode === 'chart' && <EmotionChartView refreshKey={refreshKey} />}
          </div>
        </div>
      </div>
