...
import { fetchChartStats } from '../../../services/diaryApi';
import type { ChartDataPoint } from '../../../services/diaryApi';
...
export function EmotionChartView({ refreshKey }: EmotionChartViewProps = {}) {
   ...
   * 시퀀스: 기간 타입 변경/refreshKey 변경 -> loadChartData 실행 -> fetchChartStats API 호출 -> 차트 데이터 수신 -> 화면에 표시
   * 
   * 기간 타입(주간/월간) 또는 refreshKey가 변경될 때
   * 해당 기간의 차트 데이터를 조회하여 표시
   */
  useEffect(() => {
    loadChartData();
  }, [periodType, refreshKey]);

  /**
   * 차트 데이터 로드
   * 선택된 기간 타입에 따라 최근 7일(주간) 또는 30일(월간) 데이터를 조회
   */
