 ...
 * 
 * 유스케이스: UC-27 일기 검색
 * 시퀀스: DiaryBook -> DiaryListPage -> 검색 조건 입력 -> searchDiaries API 호출 -> 검색 결과 표시 -> 날짜 클릭 -> 일기 상세 조회
 * 
 * 주요 기능:
 * - 키워드 검색 (제목, 내용)
 * - 기간 검색 (시작일, 종료일)
 * - 감정 카테고리 필터링
 * - 페이지네이션
 * - 검색 결과에서 날짜 클릭 시 일기 상세 조회
 */
import { useState, useEffect } from 'react';
import { Search, Filter, ChevronLeft, ChevronRight, Calendar, Loader2, X } from 'lucide-react';
import { searchDiaries } from '../../../services/diaryApi';
import type { DiarySearchParams, DiarySearchResult, DiaryDetail } from '../../../services/diaryApi';
...
export function DiaryListPage({ onDateClick }: DiaryListPageProps) {
  ...
  /**
   * 유스케이스: UC-27 일기 검색
   * 시퀀스: 페이지 변경/감정 카테고리 변경 -> DiaryListPage -> searchDiaries API 호출 -> 검색 결과 수신 -> 화면에 표시
   * 
   * 페이지 번호나 감정 카테고리가 변경될 때
   * 검색 조건에 맞는 일기 목록을 조회하여 표시
   */
  useEffect(() => {
    performSearch();
