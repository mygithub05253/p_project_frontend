 ...
 * CalendarPage.tsx
 * 
 * 유스케이스: UC-22 월별 감정 조회, UC-11 날짜 선택
 * 시퀀스: CalendarPage 마운트/월 변경 -> fetchMonthlyEmotions API 호출 -> 감정 데이터 로드 -> 캘린더에 감정 이모지 표시 -> 날짜 클릭 -> onDateSelect 콜백 호출
 * 
 * 주요 기능:
 * - 월별 캘린더 표시 및 월 네비게이션
 * - 각 날짜별 감정 이모지 표시 (일기가 있는 날짜)
 * - 날짜 선택 시 일기 상세 조회 페이지로 이동
 * - 오늘 날짜 하이라이트
 * - 선택된 날짜 표시
 */
import { useState, useEffect } from 'react';
import { ChevronLeft, ChevronRight, Loader2 } from 'lucide-react';
import { fetchMonthlyEmotions } from '../../../services/diaryApi';
import type { EmotionData } from '../../../services/diaryApi';
...
export function CalendarPage({ onDateSelect, selectedDate, currentMonth, onMonthChange, refreshKey, compact = false, isRightPage = false }: CalendarPageProps) {
   ...
   * 시퀀스: 월 변경/refreshKey 변경 -> CalendarPage -> fetchMonthlyEmotions API 호출 -> 감정 데이터 수신 -> 감정 맵 생성 -> 캘린더에 표시
   * 
   * 표시 중인 월이 변경되거나 데이터가 새로고침될 때
   * 해당 월의 감정 데이터를 조회하여 캘린더에 감정 이모지를 표시
   */
  useEffect(() => {
    loadMonthlyEmotions();
